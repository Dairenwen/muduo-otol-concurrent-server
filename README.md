...
std::regex pattern(R"(\d+)"); //R"()" 是 C++ 的原始字符串
```
...
项目中用于http请求的解析，主要是对请求行和请求头进行解析，使用正则表达式匹配请求行和请求头的格式，提取出请求方法、URL、HTTP版本、请求头字段等信息。正则表达式如下：

```cpp
std::regex pattern(
    "(GET|HEAD|POST|PUT|DELETE) ([^?\\s]*)(?:\\?([^\\s]*))? (HTTP/1\\.[01])\\r\\n([\\s\\S]*)"
);
```
...
```text
GET /search?name=tom&age=20 HTTP/1.1\r\n
Host: localhost\r\n
...
```cpp
match[0] // 整个匹配到的 HTTP 报文
match[1] // GET
match[2] // /search
match[3] // name=tom&age=20
match[4] // HTTP/1.1
...
- 实现思想：
  ...
  - 2.缓冲区中包含空间的大小、读取数据的位置以及写入数据的位置；
  - 3.向buffer中写入数据，从写入指针开始写入，注意可以将数据插入到前面空闲部分，如果没有空闲空间，则需要进行扩容，完成后修改写入指针；
  - 4.从buffer中读取数据，从读取指针开始读取，`可读数据大小=写入指针-读取指针`；
- 包含方法：
  - 获取当前写位置地址；
  - 确保可写空间足够；
  - 获取头部空闲空间大小；
...
- 实现思想：
  ...
  - 2.封装套接字的属性设置，如地址复用、Nagle算法、保持连接、非阻塞等；
  - 3.封装套接字的读写操作，提供对底层系统调用的封装；
- 包含方法：
  - 创建监听套接字；
  - 绑定地址信息；
  - 开始监听；
